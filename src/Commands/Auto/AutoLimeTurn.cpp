...
#include "AutoLimeTurn.h"


AutoLimeTurn::AutoLimeTurn() {
...
void AutoLimeTurn::Execute() {
	m_instance = LimelightCamera::GetInstance();

	m_cur_angle = m_instance->GetCameraData()->xOffSet;
	m_angle_err = 0 - m_cur_angle;
...
bool AutoLimeTurn::IsFinished() {
	return m_isFinished || IsTimedOut();
}

// Called once after isFinished returns true
void AutoLimeTurn::End() {
	m_isFinished = false;
}

// Called when another command which requires one or more of the same
// subsystems is scheduled to run
void AutoLimeTurn::Interrupted() {
