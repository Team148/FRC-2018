#include <Commands/Auto/AutoLimeDrive.h>
#include <Util/UnitMaster.h>
#include <iostream>

UnitMaster unit_master_convs;

AutoLimeDrive::AutoLimeDrive( double cruise_velocity, double final_velocity) {
	// Use Requires() here to declare subsystem dependencies
	// eg. Requires(Robot::chassis.get());
	Requires(Drivetrain::GetInstance());
	Requires(LimelightCamera::GetInstance());

	m_travelDistance = GetDistance();
...
AutoLimeDrive::AutoLimeDrive(double cruise_velocity, double final_velocity, double heading) {
	// Use Requires() here to declare subsystem dependencies
	// eg. Requires(Robot::chassis.get());
	Requires(Drivetrain::GetInstance());
	...
	std::cout << "is constructing overload" << m_travelDistance << cruise_velocity << final_velocity <<  std::endl;
...
bool AutoLimeDrive::IsFinished() {
	return m_isFinished;
}


double AutoLimeDrive::GetDistance(){

}

// Called once after isFinished returns true
void AutoLimeDrive::End() {
	...
	//Drivetrain::GetInstance()->configDrivetrain();
	Drivetrain::GetInstance()->Arcade(0,0);
}

// Called when another command which requires one or more of the same
// subsystems is scheduled to run
void AutoLimeDrive::Interrupted() {
	End();
}
