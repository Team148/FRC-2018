...
public:
	PathExecuter(TrajectoryPath* trajectory, bool IsReversed, int index_short = 0, int disabledPositionTimeFrames = 0);
	void Initialize();
	void Execute();
	bool IsFinished();
	void End();
...
private:
	...
	int m_disabledPositionTimeFrames = 0;
	bool m_correction_enable = true;
	int m_index_short = 0;
